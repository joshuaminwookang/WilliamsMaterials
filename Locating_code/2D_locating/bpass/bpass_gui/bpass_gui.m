function varargout = bpass_gui(varargin)
% 
% NAME:
%               bpass_gui
% PURPOSE:
%               Provides a visual way to optimise the parameters when
%               you're using bpass on an image. Creates a gui with the
%               image that you would like to bpass. Then has three slider
%               bars for the three bpass options (lnoise,lobject,threshold)
...
%               Image Processing
% CALLING SEQUENCE:
%               a=bpass_gui(image_name)
% INPUTS:
%               image_name:  The file name of the image to be filtered.
%
% OUTPUTS:
%               a:    A vector containing the three values of lnoise,
%               lobject and threshold to be used with bpass.
% NOTES:
...
%               Written by Rob Style, ETH, Nov 2016

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @bpass_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @bpass_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bpass_gui is made visible.
function bpass_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bpass_gui (see VARARGIN)
...
% Choose default command line output for bpass_gui
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes bpass_gui wait for user response (see UIRESUME)
 uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = bpass_gui_OutputFcn(hObject, eventdata, handles) 
