...
% locating, see K. E. Jensen & N. Nakamura (RSI 2016). It is also described in
...
addpath '~/Documents/Matlab/Kilfoil tracking/3D_feature_finding_algorithms/'
...
range = [1 1e5]; %to load in all available image slices, set range(2) to a large number (or Inf); software will automatically move on when it runs out of images to load
...
image_filename = 'Example1_glass.tif';
...
%folder = '~/Documents/iterative_particle_locating_examples/';
...
masscut_initial = 3e4; % first iteration generally only finds real particles to start with
