...
%Last revisement is done on June 15, 2005. Additional comments added by Rob
%Style Jan 2017. See example_kilfoil_code.m for a clarifying example
% 		Feature3d
% PURPOSE:
...
% 		Image Processing
%  CALLING SEQUENCE:
% 		f = feature3dMB(image, diameter,masksz, xyzmax,inputv, sep, masscut, threshold)
%  INPUTS:
...
% 			isotropic, a 3-vector otherwise. e.g. if the features are typically [10
% 			12 8] in size, you might choose diameter=[15 18 12];
%       masksz: the code works by first finding the feature centres to pixel level
%           accuracy. Then it applies an ellipsoidal mask around these points
%          and using the information within these masks to find the centroids
%           to sub-pixel level accuracy. This mask must be smaller than the
%           size of the features so that they don't overhand the edge of the
%           features. Thus if features are [10 12 8], you might choose
%           something like [5 6 4];
%       xyzmax: the size of the image. You can just use size(image) here.
%           inputv: if you are using any of the three optional parameters sep,
%           masscut or threshold, then you signify which with this. e.g. [1 0 0
%           ] says that you are giving a value for sep, but not for the others.
%           [0 1 1] would mean no value sep, but values for masscut and
%           threshold. If you are not giving a value for sep, then just set
%           sep=0...
% 		sep: an optional parameter which specifies the 
% 			minimum allowable separation between feature 
% 			centers. The default value is diameter-1. I'd just recommend
% 			making it the same size as the features in the figure (e.g.
% 			features that are [10 12 8], make this [10 12 8]
% 		masscut: Setting this parameter saves runtime by reducing
% 			the runtime wasted on low mass 'noise' features. %only those
% 			features with a total mass higher than masscut are considered
% 			to be a real feature. See the example_kilfoil_code.m for how
% 			you might choose this appropriately.
...
% 		'feature3dMB' is capable of finding image features with sub-pixel
...
%%%%%%%%%%%%%%%% kej adds a plot of the integrated intensities:
figure(99999); hist(m,[0:1e3:max(m)]); hold on; plot(masscut*[1 1],[0 100],'r-','LineWidth',3,'DisplayName','current cutoff'); legend('show'); xlabel('found particle masses'); hold off; beep; pause(0.5);
...
for j=1:20
    display(['fracshift iteration: ' num2str(j) ' out of 20'])
  for i=1:nmax
